...
void AAzr_Pawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent) {
    ...
    if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
        if (IA_Move) EnhancedInputComponent->BindAction(IA_Move, ETriggerEvent::Triggered, this, &AAzr_Pawn::Move);
        if (IA_Look) {
    ...
    auto ProcessLaser = [PC](UWidgetInteractionComponent* WIC,
        ...
        UAzr_ExplainWidget*& LastHoveredWidget) {
            ...
            if (WIC && WIC->IsOverHitTestVisibleWidget()) {
                ...
                // DYNAMIC GAP LOGIC
                // Set this to the large gap you want (e.g., 30.0f or 40.0f)
                float DesiredGap = 400.0f;

                // The gap will be your DesiredGap, UNLESS you are super close.
                // Multiplying by 0.5f means the gap will never consume more than half the total distance.
                float DynamicGap = FMath::Min(DesiredGap, ExactDistance * 0.5f);

                // We still keep a tiny 1.0f absolute minimum just to keep the mesh alive in memory
                float VisualBeamLength = FMath::Max(1.0f, ExactDistance - DynamicGap);

                // Ask Unreal for the exact base mesh height
                ...
                // Stretch the Z axis perfectly using the mesh's actual native height
                ...
                // Push the impact point out by 1.5 units along the surface normal
                FVector SafeHitPoint = Hit.ImpactPoint + (Hit.ImpactNormal * 1.5f);

                // Convert that exact world location into local space
                FVector LocalHitPoint = Reticle->GetAttachParent()->GetComponentTransform().InverseTransformPosition(SafeHitPoint);

                // Set relative location to sync perfectly with VR Late Updates
                Reticle->SetRelativeLocation(LocalHitPoint);

                // Make the reticle lay perfectly flat against the UI surface
                Reticle->SetWorldRotation(Hit.ImpactNormal.Rotation());

                // --- 3. AIM ON HAPTIC PULSE ---
                if (!bWasHovering) {
                    if (PC) {
                        PC->SetHapticsByValue(1.0f, 1.0f, Hand); // Turn on

                        // Turn off after a tiny delay to create a "bump" sensation
...
void AAzr_Pawn::RightStickInput(const FInputActionValue& Value) {
    ...
    // --- 2. ROOM-SCALE PIVOT ROTATION (PURE MATH OPTIMIZATION) ---
    if (FMath::Abs(TurnYaw) > 0.001f) {
        // Calculate the vector from the Pawn's true center to the Headset
        FVector CameraOffset = Camera->GetComponentLocation() - GetActorLocation();
        CameraOffset.Z = 0.0f; // Keep it strictly 2D on the floor

        // Mathematically calculate where that offset WILL be after the rotation
        FVector RotatedOffset = CameraOffset.RotateAngleAxis(TurnYaw, FVector::UpVector);

        // The exact difference is how far we need to slide the Pawn to keep the headset still
        FVector CorrectionOffset = CameraOffset - RotatedOffset;

        // Apply both the rotation and the slide simultaneously
        AddActorLocalRotation(FRotator(0.0f, TurnYaw, 0.0f));
        AddActorWorldOffset(CorrectionOffset);
    }
    ...
    if (AxisInput.Y > 0.5f) {
        if (ForwardInput == EForwardBehavior::Teleport && CachedTeleportComp)
            CachedTeleportComp->HandleTeleportInput(AxisInput.Y, RightMotionController);
    ...
    if (FMath::Abs(AxisInput.X) < 0.2f && FMath::Abs(AxisInput.Y) < 0.2f) {
        bReadyToSnapTurn = true;
        if (ForwardInput == EForwardBehavior::Teleport && CachedTeleportComp)
            CachedTeleportComp->HandleTeleportInput(0.0f, RightMotionController);
    }
}
...
void AAzr_Pawn::Move(const FInputActionValue& Value) {
    ...
    if (Controller) {
        FVector ViewFwd = Camera->GetForwardVector(); ViewFwd.Z = 0; ViewFwd.Normalize();
        FVector ViewRt = Camera->GetRightVector(); ViewRt.Z = 0; ViewRt.Normalize();
        AddActorWorldOffset(((ViewFwd * MovementVector.Y) + (ViewRt * MovementVector.X)) * SmoothMoveSpeed * GetWorld()->GetDeltaSeconds(), true);
    }
}
