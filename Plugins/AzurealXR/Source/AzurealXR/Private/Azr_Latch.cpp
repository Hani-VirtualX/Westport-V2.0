...
void UAzr_Latch::DisableLatch()
{
	...
	// Multi-hand cleanup. Release every hand in the array to prevent "ghost" grabs
...
void UAzr_Latch::SetLatchConfig(EAzr_LatchType NewType, EAzr_Axis NewAxis, float NewMin, float NewMax, float NewStartValue)
{
	...
	// 2. THE MEMORY WIPE
	// We capture exactly where the handle is sitting *right now* and make it the new absolute zero point.
	InitialTransform = GetRelativeTransform();

	// 3. Reset the math tracker so it counts freshly from this new zero point
	float PreConfigValue = CurrentRawValue;
	CurrentRawValue = 0.0f;

	// 4. If the latch is currently enabled, snap it to the new StartValue immediately
	if (bIsGrabEnabled)
	{
		CurrentRawValue = FMath::Clamp(StartValue, LimitMin, LimitMax);
...
void UAzr_Latch::ReleaseSpecificHand(USceneComponent* Hand)
{
	...
	// UPDATED: Explicitly check the ecosystem split boolean
	USceneComponent* GhostHand = nullptr;
	if (bUseLatchSnapHand)
	{
		GhostHand = bIsRight ? CustomRightSnap : CustomLeftSnap;
	...
	if (ActiveHands.Num() == 0)
	{
		...
		if (bIsGrabEnabled)
		{
			...
			// (NOTE: SpawnedLatchIndicator visibility code has been deleted. Zone handles UI.)
		}
		OnLatchReleased.Broadcast();
	}
	else if (bWasLeader)
	{
		// Hand off lead math to the remaining hand
