...
void UAzr_Explain::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction) {
    ...
    // --- 1. AUDIO PROGRESS MATH ---
    if (ActiveAudioComp && ActiveAudioComp->IsPlaying() && ActiveExplainUI) {
        if (ActiveAudioComp->ComponentTags.Num() >= 2) {
            float Duration = FCString::Atof(*ActiveAudioComp->ComponentTags[0].ToString());
            float StartTime = FCString::Atof(*ActiveAudioComp->ComponentTags[1].ToString());
...
void UAzr_Explain::HandlePlayAudioClicked() {
    ...
    if (TrackToPlay && ActiveWidgetComp) {
        ...
        ActiveAudioComp->ComponentTags.Empty();
        ActiveAudioComp->ComponentTags.Add(FName(*FString::SanitizeFloat(TrackToPlay->GetDuration())));
        ActiveAudioComp->ComponentTags.Add(FName(*FString::SanitizeFloat(GetWorld()->GetTimeSeconds())));
    }
    else if (ActiveExplainUI) {
        ActiveExplainUI->SetPlaybackCompleted();
    }
}
