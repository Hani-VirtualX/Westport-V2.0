...
class UAzr_ExplainWidget; // Added so the Pawn knows what this is
...
protected:
    ...
    void PassTriggerToHeldObject(UMotionControllerComponent* HandController,
        float Value);

    void ProcessGazeTrace();

    // --- NEW: LASER TICK HANDLER ---
    ...
    // --- NEW: MEMORY POINTERS FOR CLEANUP ---
    UPROPERTY()
